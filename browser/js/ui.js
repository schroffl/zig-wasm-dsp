...
const CustomUI = {
    ...
    slider: function (elem, config) {
        ...
        CustomUI.registerDragListener(slider_handle, {
            ...
            onend: (ref) => {
                ...
                if (typeof config.ondragend === 'function') config.ondragend();
            },
        });
    ...
    registerDragListener: function (elem, config, add_no_touch_css) {
        ...
        if (add_no_touch_css !== false) elem.classList.add('no-touch-action');
        ...
        elem.addEventListener(
            ...
            (e) => {
                ...
                if (typeof config.onstart === 'function') config.onstart(config.ref, e);
            },
            { passive: false },
        );
        ...
        const stop_dragging = (e) => {
            if (elem.hasPointerCapture(e.pointerId)) {
                ...
                if (typeof config.onend === 'function') config.onend(config.ref, e);
            }
        };
    ...
    registerHotkeys: function (elem, hotkey_map) {
        ...
        elem.addEventListener('keydown', (e) => {
            if (e.key === ' ' || e.key === 'Enter') {
                ...
                if (node_name === 'button' && e.target !== elem) return;
            }
    ...
    nextFrame: function (f) {
        ...
        if (stack.indexOf(f) === -1) stack.push(f);
...
document.addEventListener('selectstart', (e) => {
    if (CustomUI.prevent_text_selection) e.preventDefault();
});
