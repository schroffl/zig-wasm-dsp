...
const mode_ctrl = lissajous_settings.add(settings, 'mode', {
    '3D': '3d',
    '2D': '2d',
    Heatmap: 'heatmap',
});

const ring_size_ctrl = lissajous_settings
    .add(settings, 'ring_size', 1, audio_ctx.sampleRate * 20, 1)
    .name('Buffer Size');
...
input_settings
    .add(settings, 'gain', 0, 10, 0.01)
    .name('Gain')
    .onChange((value) => (gain_node.gain.value = value));
input_settings
    .add(settings, 'monitor_volume', 0, 1, 0.01)
    .name('Monitor Volume')
    .onChange((value) => (out_gain_node.gain.value = value));
...
function selectFile(accept, onchange) {
    ...
    const removeInput = () =>
        file_input.parentElement && file_input.parentElement.removeChild(file_input);
    file_input.onchange = () => {
        removeInput();
    ...
    document.body.onfocus = (e) => {
        ...
        if (typeof old_on_focus === 'function') old_on_focus(e);
    };
...
const loop = animationLoop((t) => {
    ...
    lissajous_pane.config.dots.color = {
        r: settings.dot_color[0],
        g: settings.dot_color[1],
        b: settings.dot_color[2],
    };
